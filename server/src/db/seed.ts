...
export async function seedIfEmpty(): Promise<void> {
  const { count } = (await db.prepare("SELECT COUNT(*) as count FROM books").get()) as {
    count: number;
  };
  if (count > 0) return;
  ...
  `);
  const insertShelf = db.prepare("INSERT INTO shelf (book_id, status) VALUES (?, 'read')");
  ...
  for (const book of seedCatalog) {
    ...
    if (book.onShelf) await insertShelf.run(book.id);
  }
}
